...
支持任意 Causal LM:
  - GPT2-Chinese:   uer/gpt2-chinese-cluecorpussmall (100M)
  - Qwen2-1.5B:     Qwen/Qwen2-1.5B-Instruct   (1.5B, 对话模型)

通过 config.gpt2_model_name 切换。
...
class PretrainedLM(nn.Module):
    ...
    def freeze_bottom_layers(self, n: int):
        ...
        # 获取 transformer layers（兼容 GPT2 和 Qwen2）
        if hasattr(self.model, "transformer") and hasattr(self.model.transformer, "h"):
            layers = self.model.transformer.h  # GPT2
    ...
    def apply_chat_template(self, messages: list[dict], add_generation_prompt: bool = True) -> str:
        """应用模型自带的 chat template（Qwen2 等有，GPT2 没有则回退到简单格式）。"""
...
if __name__ == "__main__":
    import sys
    model_name = sys.argv[1] if len(sys.argv) > 1 else "uer/gpt2-chinese-cluecorpussmall"
