...
#  GPT Decoder-Only 解码器


class GPTDecoder:
    """GPT Decoder-Only 解码器，支持 Beam Search / 采样 / 贪心解码。"""
    ...
    def beam_search(
        ...
        """
        Beam Search 解码（Decoder-Only 自回归生成）。

        参数:
            prompt_ids: prompt 序列 token ID 列表（如 [SOS, Q..., EOS]）
            beam_size: Beam 宽度
            max_len: 最大解码长度（新生成 token 数）
        ...
        # 初始化 Beam: 每个候选为 (token_ids 完整序列, log_prob_sum, is_done)
        ...
        for _ in range(max_len):
            ...
            for seq, score, is_done in beams:
                ...
                # 自回归前向：输入完整序列，取最后位置 logits
                input_tensor = torch.tensor([seq], device=device)
                pad_mask = torch.zeros(1, len(seq), dtype=torch.bool, device=device)
                logits = self.model(input_tensor, pad_mask)  # (1, seq_len, vocab_size)
                ...
                for token_id, token_log_prob in zip(top_ids.tolist(), top_scores.tolist()):
                    ...
                    if token_id == self.eos_id:
                        # 去掉 prompt 和 EOS，只保留新生成部分
                        completed.append((new_seq[prompt_len:-1], new_score))
                    else:
                        new_candidates.append((new_seq, new_score, False))
            ...
            for seq, score, is_done in new_candidates:
                if not is_done:
                    gen_len = len(seq) - prompt_len + 1  # +1 避免初始为 0
        ...
        # 收集未完成的 beam
        for seq, score, is_done in beams:
            if not is_done and len(seq) > prompt_len:
                completed.append((seq[prompt_len:], score))

        if not completed:
            if beams:
                completed = [(beams[0][0][prompt_len:], beams[0][1])]
            else:
                completed = [([], 0.0)]
    ...
    def sample(
        ...
        for _ in range(max_len):
            ...
            pad_mask = torch.zeros(1, len(generated), dtype=torch.bool, device=device)
            logits = self.model(input_tensor, pad_mask)
            logits = logits[0, -1] / temperature
        ...
        return self._decode_ids(generated[prompt_len:])

    @torch.no_grad()
    def greedy(self, prompt_ids: list[int], max_len: int | None = None) -> str:
        """贪心解码（beam_size=1 的特例）。"""
        results = self.beam_search(prompt_ids, beam_size=1, max_len=max_len)
...
#  交互式聊天 (Encoder-Decoder)


class ChatBot:
    """交互式中文聊天机器人（Encoder-Decoder 架构）。"""
...
#  交互式聊天 (GPT Decoder-Only)


class GPTChatBot:
    """GPT 中文聊天机器人，支持多轮对话记忆（Decoder-Only 架构）。"""
    ...
    def _build_prompt(self, text: str) -> list[int]:
        ...
        格式: [SOS] Q1 [EOS] R1 [EOS] [SOS] Q2 [EOS]
        模型从当前 query 的 EOS 之后开始生成回复。

        max_history=0 时不添加历史（单轮模式）。
        """
        prompt: list[int] = []

        # 添加历史对话（受 max_history 限制）
        history_to_use = self.history
        if self.max_history > 0:
            history_to_use = self.history[-self.max_history:]

        for q_ids, r_ids in history_to_use:
        ...
        # 添加当前 query
        q_ids = self._tokenize(text)
        prompt.append(SOS_ID)
        prompt.extend(q_ids)
        prompt.append(EOS_ID)

        # 预留 max_decode_len 空间给生成，避免序列超出 PE 的 max_len
        max_prompt_len = max(self.config.max_len - self.config.max_decode_len, 10)
    ...
    def reply(self, text: str, use_beam: bool = True, use_sample: bool = False) -> str:
        ...
        if len(prompt_ids) <= 2:  # 只有 SOS + EOS，无有效输入
            return "请输入有效内容~"

        if use_sample:
            response = self.decoder.sample(
                prompt_ids,
                temperature=self.config.temperature,
                top_k=50,
                top_p=0.9,
            ...
            results = self.decoder.beam_search(prompt_ids)
            response = results[0][0] if results else "我不知道怎么回...T_T"
        else:
            # 贪心解码
            response = self.decoder.greedy(prompt_ids)

        # 保存到历史
        q_ids = self._tokenize(text)
        r_ids = self._tokenize(response) if response else []
        self.history.append((q_ids, r_ids))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Chinese Transformer ChatBot")
    parser.add_argument(
        "--device",
        type=str,
        choices=["cpu", "cuda"],
        help="运行设备：cpu 或 cuda（默认使用 config.device）"
    )
    parser.add_argument(
        "--model",
        type=str,
        choices=["transformer", "gpt"],
        help="模型架构: transformer (Encoder-Decoder) | gpt (Decoder-Only)"
    )
...
def _detect_model_type(checkpoint_path: str) -> str:
    """
    从 checkpoint 的 state_dict 自动检测模型架构。

    返回: "transformer" 或 "gpt"
    """
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    state_dict = checkpoint.get("model_state_dict", checkpoint)
    # GPT: embedding.weight（无前缀）
    # Transformer: encoder.embedding.weight（有 encoder/decoder 前缀）
    sample_key = next(iter(state_dict.keys()))
    if sample_key.startswith("encoder.") or sample_key.startswith("decoder."):
        return "transformer"
    return "gpt"
...
def main():
    ...
    # 确定模型路径
    model_path = os.path.join(config.model_save_dir, config.model_path)
    if not os.path.exists(model_path):
        ...
        if os.path.exists(config.model_save_dir):
            ...
            if ckpts:
                ...
                print(f"[Warning] 未找到 {config.model_path}，使用: {ckpts[0]}")
            else:
    ...
    print(f"[Info] 模型架构: {config.model_type}")
